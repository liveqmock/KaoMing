 ...
 * @author xt
 * @version 1.1.5.6
 * @see StockInfoListQuery.java <br>
 */
public class StockInfoListQuery extends QueryBean{

	public StockInfoListQuery(ActionForm form){
		super(form);
	}

	public StockInfoListQuery(ActionForm form, String start, String end) {
	...
	private ArrayList queryCondition(StockInfoForm form){
		...
		if(form!=null){
			...
			if (form.getStrSkuNum() != null && !form.getStrSkuNum().equals("")) {
				strHaving +=  " and sum(si.skuNum) = "+form.getStrSkuNum();
				
			}

			if (form.getStrPerCost() != null && !form.getStrPerCost().equals("")) {
				strHaving +=  " and avg(si.perCost) = :strPerCost";
				QueryParameter param = new QueryParameter();
	...
	protected AdvQueryString generateListQuery(ActionForm aform) {
		...
		String queryString = "select si.stuffNo,si.skuCode," +
			" sum(si.skuNum),sum(si.perCost * si.skuNum)/sum(si.skuNum), " +
			" sum(si.orderDollar * si.skuNum)/sum(si.skuNum),si.skuType " +
			" from StockInfoForm as si  "+whereIn+
			" group by si.stuffNo,si.skuCode,si.skuType " +strHaving+
			" order by si.stockId";
