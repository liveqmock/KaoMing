...
<%
	try{
		ArrayList loanPartOutList = (ArrayList)request.getAttribute("loanPartOutList");
		int count=0;
		if(loanPartOutList!=null){
			count=Integer.parseInt((String)loanPartOutList.get(0));
		}
		List createByList=RepairOperateBo.getInstance().getPartCreateByList();

%>
<html:form action="repairOperateAction.do?method=loanPartOutList" method="post" >
...
<br>
  <table width="100%" border="0" cellpadding="0" cellspacing="1" class="content12">
	...
	<%
       if(loanPartOutList!=null){
       	    String strTr="";
      	    for(int i=1;i<loanPartOutList.size();i++){
      	        if(i%2==0) strTr="tableback2";
      	        else strTr="tableback1";
      		String[] temp=(String[])loanPartOutList.get(i);
      %>      
      <tr class="<%=strTr%>"> 
...
<script language="JavaScript">
	...
	function f_print(){
	    ...
	    if(chk()!=''&&chk()!=null){
	    		...
	    		document.forms[0].action="repairOperateAction.do?method=stockOutConfirm&partType=X";
